...
cd /usr/local
ls >> /root/report.json
#List /usr/local/little-paper
echo "############Filelist of /usr/local/little-paper" >> /root/report.json
...
if [ ! -f "/usr/local/bin/ssr" ]; then
  echo "little-paper主文件缺失，请确认服务器是否成功连接至Github"
  echo "SSR Miss" >> /root/report.json
  exit
fi
if [ ! -f "/usr/local/little-paper/server.sh" ]; then
  echo "little-paper主文件缺失，请确认服务器是否成功连接至Github"
  echo "SSR Miss" >> /root/report.json
  exit
fi
